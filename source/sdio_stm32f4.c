...
/*!
    ...
    @version        23.00
    @date           2018.11.28
	@brief          SDIO Driver For FatFs and STM32F4 Devices			@n
					Based on STM324xG-EVAL SD-Sample V1.0.2. Thanks!

    @section HISTORY
		2018.11.28	V23.00	See sdio_stm32f4_ver.txt.
...
static SD_Error SD_ReadSmart(uint8_t *SmartDat);
#endif
...
SD_Error SD_Init(void)
{
	...
	uint8_t eext_csd[512];
	memset(eext_csd,0,sizeof(eext_csd));
	if(SD_ReadSmart(eext_csd) == SD_OK)
	{
		for(int i =0; i<512 ;i++) printf("smart[%d] = %d : %c \n",i,eext_csd[i],eext_csd[i]);
	}
#endif
...
SD_Error SD_PowerON(void)
{
	...
	{	/*!< else MMC Card */
		...
		while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
		{
			...
			validvoltage = (((response >> 31) == 1) ? 1 : 0);
			count++;
			MMC_CMD6_WAIT(); /* Need for MMCv5 or above devices */
		}
	...
	switch (drv) 
	{
		case SDIO_DRIVE:
		{
		  ...
		  switch (ctrl)
		  {
			...
			case SD_GET_SCR :	/* Read SCR (8 bytes) */
...
/**************************************************************************/
/*! 
  * @brief  Read S.M.A.R.T. Datas for Delkin.
  * @param  hsd: SD handle
  * @param  SmartDat: pointer to the 512Byte SMART Data Structure.  
  * @retval SD Card error state
*/
/**************************************************************************/
SD_Error SD_ReadSmart(uint8_t *SmartDat)
{
	SD_Error errorstatus = SD_OK;
	uint32_t count = 0, *tempbuff = (uint32_t *)SmartDat;
	...
	/*!< CMD56 -Read- */
	SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)READ_SMART_CMD56;
	SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GEN_CMD;
	SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
	SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
	SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
	SDIO_SendCommand(&SDIO_CmdInitStructure);
