...
#if __MMC_STM32F4_H	!= 0x1600
#error "header file version is not correspond!"
#endif

/* Defines -------------------------------------------------------------------*/
/* Assigned to CoreCoupled-RAM(ccram/stack section) */
...
#define CMD56	(56)		/* GEN_CMD(used for reading S.M.A.R.T. data) */
#define CMD58	(58)		/* READ_OCR */

/* MMC_DMA Direction FLAGS */
enum{ 
		MMCDMA_TX_TRANS = (0),
		MMCDMA_RX_TRANS = (1)
	};
/* ACMD41 Argument for SDXC  |HCS(Bit30) | S18R(Bit24) | */  
/* XPC doesn't support on SPI Mode */
#define ACMD41_ARG			((1UL << 30) | (1UL << 24))

/* Variables -----------------------------------------------------------------*/
static volatile DSTATUS Stat = STA_NOINIT;		/* Disk status */
static volatile unsigned int Timer1, Timer2;	/* 100Hz decrement timer */
static uint8_t CardType;						/* Card type flags */
uint8_t csd[16];								/* For STM32F4 DMA handling */

#ifdef USE_SPIMMC_DMA
/* MMCDMA Structures */
static DMA_InitTypeDef MMCDMA_InitStructure __ATTR_CCRAM;
/* For MMC Rx STM32F4 DMA Dummy Buffer */
const uint8_t rw_dummybyte =0xFF; 				
#endif
...
static void FCLK_FAST(void)
{
	...
	tmp = SPIMMC->CR1;
	tmp = ( tmp & ~SPI_BaudRatePrescaler_256 ) | SPIMMC_MAXBAUD;	/* Should set to below 25MHz */
	SPIMMC->CR1 = tmp;
}
...
{
	/* Loop while DR register in not empty(may not needed) */
	/* while (SPI_I2S_GetFlagStatus(SPIMMC, SPI_I2S_FLAG_TXE) == RESET) { ; } */

	/* Send byte through the SPIMMC peripheral */
	SPIMMC->DR = out;

	/* Wait to complete receive a byte */
	while (!(SPIMMC->SR & SPI_SR_RXNE)) { ; }
...
static void power_on(void)
{
	...
	/* Configure I/O for SD/MMC Chip select(GPIO Controlled) */
	GPIO_InitStructure.GPIO_Pin		= SPIMMC_PIN_CS;
	GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed 	= GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_OType 	= GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd 	= GPIO_PuPd_NOPULL;		/* MUST be pulluped by external register */
	...
	MMCDMA_InitStructure.DMA_Channel 				= SPIMMC_DMA_CHANNEL_RX;  
	MMCDMA_InitStructure.DMA_PeripheralBaseAddr 	= (uint32_t)(&(SPIMMC->DR));
	MMCDMA_InitStructure.DMA_PeripheralInc 			= DMA_PeripheralInc_Disable;
	MMCDMA_InitStructure.DMA_PeripheralDataSize 	= DMA_PeripheralDataSize_Byte;
	MMCDMA_InitStructure.DMA_MemoryDataSize 		= DMA_MemoryDataSize_Byte;
	MMCDMA_InitStructure.DMA_Mode 					= DMA_Mode_Normal;
	MMCDMA_InitStructure.DMA_Priority 				= DMA_Priority_VeryHigh;
	MMCDMA_InitStructure.DMA_FIFOMode 				= DMA_FIFOMode_Disable;
	MMCDMA_InitStructure.DMA_FIFOThreshold 			= DMA_FIFOThreshold_Full;
	MMCDMA_InitStructure.DMA_MemoryBurst 			= DMA_MemoryBurst_Single;
	MMCDMA_InitStructure.DMA_PeripheralBurst 		= DMA_PeripheralBurst_Single;
#endif
}
...
static void power_off(void)
{
	...
	if (!(Stat & STA_NOINIT)) {
		CS_HIGH();
		wait_ready(50);
		deselect();/* Wait to complete receive a last byte */
	}

	/* Disable SPI Module */
	/* Wait to Tx Buffer Empty */
	while (!(SPIMMC->SR & SPI_SR_TXE)) { ; }
	/* SPI module busy check(must need on disabling SPI) */
	while ((SPIMMC->SR & SPI_SR_BSY)) { ; }
	/* Disable SPI Module */
...
static void stm32_dma_transfer(
	int transdir,		/* 0:MMCDMA_TX_TRANS
						     TX Transaction for buff->SPI
						   1:MMCDMA_RX_TRANS
	                         RX Transaction for SPI->buff*/
	const uint8_t *buff,/* TX Transaction : 512 byte data block to be transmitted
						   RX Transaction : Data buffer to store received data    */
	unsigned int  btr 	/* transdir = MMCDMA_TX_TRANS : Byte count (must be multiple of 2)
						   RX Transaction : Byte count (must be 512)              */
)
{
	/* Due to CCM-RAM(cannot DMA region) assigned to stack/auto variable*/
	...
	/* shared DMA configuration values */
	MMCDMA_InitStructure.DMA_BufferSize		 	= btr;

	if(transdir){	/* (transdir == MMCDMA_RX_TRANS): RX Transaction */
		/* DMA_SPI_RX configuration SPI_MMC RX ---------------------------------------------*/
		MMCDMA_InitStructure.DMA_Memory0BaseAddr 	= (uint32_t)buff;
		MMCDMA_InitStructure.DMA_DIR 			  	= DMA_DIR_PeripheralToMemory;
		MMCDMA_InitStructure.DMA_MemoryInc		  	= DMA_MemoryInc_Enable;
		DMA_Init(SPIMMC_DMA_STREAM_RX, &MMCDMA_InitStructure);

		/* SPI_DMA_TX configuration SPI_MMC TX ---------------------------------------------*/
		MMCDMA_InitStructure.DMA_Memory0BaseAddr 	= (uint32_t)(&rw_dummybyte);
		MMCDMA_InitStructure.DMA_DIR 				= DMA_DIR_MemoryToPeripheral;
		MMCDMA_InitStructure.DMA_MemoryInc		 	= DMA_MemoryInc_Disable;
		DMA_Init(SPIMMC_DMA_STREAM_TX, &MMCDMA_InitStructure);
	}
	else       {	/* (transdir == MMCDMA_TX_TRANS): TX Transaction */
		/* DMA_SPI_RX configuration SPI_MMC RX ---------------------------------------------*/
		MMCDMA_InitStructure.DMA_Memory0BaseAddr 	= (uint32_t)(&rw_dummybyte);
		MMCDMA_InitStructure.DMA_DIR			 	= DMA_DIR_PeripheralToMemory;
		MMCDMA_InitStructure.DMA_MemoryInc		 	= DMA_MemoryInc_Disable;
    ...
    SPIMMC_DMA_STREAM_RX->CR &= ~(uint32_t)DMA_SxCR_EN;
    SPIMMC_DMA_STREAM_TX->CR &= ~(uint32_t)DMA_SxCR_EN;
	SPIMMC->CR2 &= (uint16_t)~(SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN);
...
)
{
	...
	if(btr != 512){
		...
		stm32_dma_transfer(MMCDMA_RX_TRANS, buff, btr);
	}
#else
...
#ifdef USE_SPIMMC_DMA
		stm32_dma_transfer(MMCDMA_TX_TRANS, buff, 512);
#else
		wc = 0;
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* The card can work at vdd range of 2.7-3.6V */
				while (Timer1 && send_cmd(ACMD41, ACMD41_ARG));	/* Wait for leaving idle state (ACMD41 with HCS bit) */
...
#if defined(ENABLE_SMART_READ)
   	/* Read and Parse SMART(Test use only) */
	if (send_cmd(CMD56, READ_SMART_CMD56) == 0) {	/* Get SMART */
		uint8_t nbuf[512];
		memset(nbuf,0,sizeof(nbuf));
		uint8_t* ptr2 = nbuf;
		do {							/* Wait for data packet */
		} while (rcvr_spi() == 0xFF);
		for (int i = 512; i; i--) *ptr2++ = rcvr_spi();
		rcvr_spi();						/* Discard CRC */
		rcvr_spi();
		for(int i =0; i<512 ;i++) printf("smart[%d] = %d : %c \n",i,nbuf[i],nbuf[i]);
	}
#endif
...
)
{
	...
	case GET_SECTOR_SIZE :  /* Get R/W sector size (WORD) (needed at FF_MAX_SS != FF_MIN_SS) */
		if ((send_cmd(CMD9, 0) == 0) && rcvr_datablock(csd, 16)) {
			*(uint16_t*)buff = 1<<(csd[5] & 15);
			res = RES_OK;
		}
		break;
		...
		if (CardType & CT_SD2) {	/* SDC ver 2.00 */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				...
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					...
					else if(((csd[10] >> 4) == 11) || ((csd[10] >> 4) == 12))  {
						*(uint32_t*)buff = 16384UL + 8192UL*((csd[10] >> 4)-10);
					}
					else if(((csd[10] >> 4) == 13) || ((csd[10] >> 4) == 14))  {
						*(uint32_t*)buff = 32768UL + 16384UL*((csd[10] >> 4)-12);
					}
					else { /* (csd[10] >> 4) == 15 */
	...
	case SD_GET_SCR :		/* Read SCR status (8 bytes) */
		if (send_cmd(ACMD51, 0) == 0) {	/* READ_SCR */
			rcvr_spi();
			if (rcvr_datablock(ptr, 8))
				res = RES_OK;
		}
		break;

	case MMC_GET_EXTCSDREV :	/* Read ExtCSD revision */
		if (send_cmd(CMD8, 0) == 0) {	/* GET_ExtCSD(MMCv4) */
			uint8_t nbuf[512];
			uint8_t* ptr2 = nbuf;
			do {							/* Wait for data packet */
			} while (rcvr_spi() == 0xFF);
			...
			rcvr_spi();						/* Discard CRC */
			rcvr_spi();
			*((uint8_t *) buff + 0) = nbuf[511-192]; /* Extract EXT_CSD_REV */
			res = RES_OK;
		}
	break;
